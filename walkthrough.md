...
git status
```
When you run any command it looks for the top level `.git/` folder.
...
```
By default `git diff` compares working tree to the 
```bash
git add -u
```
...
file locally (in the working tree), e.g. if it feels like unnessary clutter for 
others to view.
```bash
...
git switch dev
```
The staging area moves with us. We can add and commit in one go, when the file is already tracked:
```bash
git commit -m "untested new idea" src/myscript.py
cat src/myscript.py
```
We can now switch back to the stable version on the default branch.
Run `git branch` to remind yourself what it was called, in my case "main".
```bash
git switch main
cat src/myscript.py
```
...
Git can also easily handle multiple people editing a file, which looks the same 
as editing it in different branches.
```bash
# prepend a shebang
...
```
There would have been no difference if this conflict was made by a 
collaborator, but in this simple branching example you can play both sides.
...
rebasing dev onto main, which means we attempt to "replay" making the changes 
in dev (since divergence) on top of the changes in main (since divergence). 
...
git rebase --continue
```
Close the editor that opens, e.g. if it is vim, then type :q.
```bash
git log --oneline --graph --branches
```
We now see the "pandas" commit has been applied on top of the "argv" commit.
Since we decided to combined the branches inside dev we have to do another step 
...
rewrites history. That can be dangerous if you collaborate with others and they 
work based on history that no longer exists.
Another powerful use of rebase is interative rebasing, where e.g.
`git rebase -i HEAD~4` let's you interatively select what to do with the last 4 
...
We could then switch back to where the dev branch has been with `git checkout 
...
## Tab-completion
Another way to get help is with tab-completion. It might already work, try 
...
```
If you want to run `data/CSA/CSA_sites.tsv.sh` (after `cd data/CSA/`) then install 
...
- Config contains your aliases, which are useful shortcuts, e.g.
  - `git config --global alias.st status` to write `git st` instead of having to do `git status`
  - `git config --global alias.unstage 'restore --staged'`
  - You can even run arbitrary shell commands with git alias, e.g. resolve conflict with `git both`
...
```
This creates a private key in plaintext, which means if someone gains access to 
your computer they can use it to access your GitHub account.
...
This is where merge conflicts may arise which we looked at earlier.
...
have been a lot of added files.
